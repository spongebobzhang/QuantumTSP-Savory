...
function read_qtsp_cli_config()
    sim_time = isempty(ARGS) ? parse(Float64, get(ENV, "QTSP_SIM_TIME", string(QTSP_DEFAULT_SIM_TIME))) : parse(Float64, ARGS[1])
    quantum_delay = length(ARGS) >= 2 ? parse(Float64, ARGS[2]) : parse(Float64, get(ENV, "QTSP_QUANTUM_DELAY", string(QTSP_DEFAULT_QUANTUM_DELAY)))
    werner_w = length(ARGS) >= 3 ? parse(Float64, ARGS[3]) : parse(Float64, get(ENV, "QTSP_WERNER_W", string(QTSP_DEFAULT_WERNER_W)))
    window_stats_interval = length(ARGS) >= 4 ? parse(Float64, ARGS[4]) : parse(Float64, get(ENV, "QTSP_WINDOW_STATS_INTERVAL", string(QTSP_DEFAULT_WINDOW_STATS_INTERVAL)))
    state_count_env = get(ENV, "QTSP_STATE_COUNT", "")
    state_count = isempty(state_count_env) ? nothing : parse(Int, state_count_env)
...
if abspath(PROGRAM_FILE) == @__FILE__
    result = run_two_node_qtsp(; read_qtsp_cli_config()...)

    println("QTSP two-node state transmission demo")
    println("  source state cap       = ", isnothing(result.state_count) ? "unbounded" : result.state_count)
    println("  simulation time        = ", result.sim_time)
    println("  flow uuid              = ", result.flow_uuid)
    println("  window size            = ", result.window_size)
    println("  states sent            = ", result.sent_states)
    println("  states received        = ", result.received_states)
    println("  states ACKed           = ", result.acked_states)
    ...
    println("  send rate              = ", result.send_rate)
    ...
    println("  quantum delay          = ", result.quantum_delay)
    println("  classical delay        = ", result.classical_delay)
    println("  source ACK timeout     = ", result.source_ack_timeout)
    println("  window stats interval  = ", result.window_stats_interval)
    println("  window samples         = ", result.window_samples)
    println("  mean window sent       = ", result.mean_window_sent)
    println("  mean window ACKed      = ", result.mean_window_acked)
    println("  mean window timeouts   = ", result.mean_window_timeouts)
    println("  total time             = ", result.total_time)
    println("  delivery throughput    = ", result.delivery_throughput)
    println("  ACKed throughput       = ", result.acked_throughput)
    println("  mean delivery time     = ", result.mean_quantum_delivery_time)
    println("  mean RTT               = ", result.mean_rtt)
    println("  mean observed fidelity = ", result.mean_observed_fidelity)

    for record in result.state_records
        println(
            "  flow ", record.flow_uuid,
            " state ", record.seq_num,
            " ", record.source_node, " -> ", record.destination_node,
            ": received = ", record.received,
            ", ACKed = ", record.acked,
            ", failed detection = ", record.failed_detection,
            ", timed out = ", record.timed_out,
            ", delivery = ", record.quantum_delivery_time,
            ", RTT = ", record.rtt,
            ", fidelity = ", record.observed_fidelity,
            ", source retain slot = ", record.source_retain_slot,
            ", source send slot = ", record.source_send_slot,
            ", destination slot = ", record.destination_slot,
        )
    end
end
